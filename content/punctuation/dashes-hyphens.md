...
Em dashes (—), en dashes (–), hyphens (-), and minus signs (−) aren't interchangeable symbols. Follow these guidelines to help you use them the right way, in the right places.
...
- **En dashes**: Use in ranges of numbers and dates, such as in time and date stamps in UI. An en dash can also be used instead of a hyphen in some compound words (see [Hyphens](hyphens) for details). Don't use in place of the minus sign. Don't use spaces around en dashes.
...
- [Hyphens](hyphens): Use to join words and connect prefixes to stem words. Don't use multiple hyphens in place of an em dash.
